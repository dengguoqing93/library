...
public class StudentInfoAction {
	...
	@RequestMapping(value="inputStudentInfo")
	@ResponseBody
	public String inputStudentInfo(String studentno) {
		Map<String, Object> map = new HashMap<String,Object>();
		if (studentno == "" || studentno == null) {
			map.put("msg", "请输入学号");
			return JSON.toJSONString(map);
		}
		StudentInfo student = studentService.selectByPrimaryKey(Long.parseLong(studentno));
		if (student==null) {
			map.put("msg", "输入的学号不存在");
			return JSON.toJSONString(map);
		}else{
